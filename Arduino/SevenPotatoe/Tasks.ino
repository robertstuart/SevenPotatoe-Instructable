...
void checkConsole() {
  while (Serial.available() > 0) {
    ...
    if (c == 'd') {
      ...
      for (int i = 0; i < tcDrive; i++) {
        Serial.printf("%d, %d\n", (int) tickLogDrive[i], (int) intLogDrive[i]);

      }
      Serial.println("--------------React-----------");
      for (int i = 0; i < tcReact; i++) {
        Serial.printf("%d, %d\n", (int) tickLogReact[i], intLogReact[i]);
      }
    } else if (c == 'e') {
      Serial.printf("Drive: %6d %6d %6d\nReact:  %6d %6d %6d\n", 
                    interruptErrorsDriveA, interruptErrorsDriveB, interruptErrorsDriveC,
                    interruptErrorsReactA, interruptErrorsReactB, interruptErrorsReactC);
    }
  }
}
 ...
 * addTickLog???()
 *****************************************************************************/
void addTickLogDrive(unsigned long time, bool isA, bool isRise) {
  tickLogDrive[tcDrive] = time;
  byte b = 100;
  if (isA) b += 1;
  if (isRise) b += 10;
  intLogDrive[tcDrive] = b;
  tcDrive++;
  if (tcDrive >= N_TICK_LOGS) tcDrive = 0;
}
void addTickLogReact(unsigned long time, bool isA, bool isRise) {
  tickLogReact[tcReact] = time;
  byte b = 100;
  if  (isA) b += 1;
  if (isRise) b += 10;
  intLogReact[tcReact] = b;
  tcReact++;
  if (tcReact >= N_TICK_LOGS) tcReact = 0;
}
